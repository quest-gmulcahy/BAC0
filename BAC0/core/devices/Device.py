...
class Device(SQLMixin):
    ...
    def new_state(self, newstate):
        ...
        """
        self.log_info('Changing device state to {}'.format(newstate))
    ...
    def add_proprietery_object(self, name, cls):
        if name in self.points_name:
            raise ValueError(
                'Name already exists, please choose something else')
    ...
    def repeat(self, func, delay=60):
        """
        Will repeat this task attached to a device 
        """
        RecurringTask(func, delay=delay).start()
...
class DeviceConnected(Device):
    ...
    def disconnect(self):
        self.log_info('Wait while stopping polling')
    ...
    def _buildPointList(self):
        ...
        try:
            ...
            self.log_error(
                'Controller not found, aborting. ({})'.format(error))
            return ('Not Found', '', [], [])

        except SegmentationNotSupported as error:
            self.log_warning('Segmentation not supported')
        ...
        self.log_info('Device {}:[{}] found... building points list'.format(
        ...
        try:
            ...
            self.log_error('Cannot retrieve object list, disconnecting...')
            self.segmentation_supported = False
            self.new_state(DeviceDisconnected)
    ...
    def connect(self, *, db=None):
        ...
        try:
            ...
            if not self.segmentation_supported or \
                ...
                self.log_debug('Segmentation not supported')
            else:
            ...
            if self.properties.db_name:
                ...
                self.log_warning("Ex. controller.connect(db = 'backup')")
...
class DeviceFromDB(DeviceConnected):
    ...
    def connect(self, *, network=None, from_backup=None):
        ...
        if network and from_backup:
            ...
            try:
                ...
                self.log_error('Unable to connect, keeping DB mode active')
    ...
    def initialize_device_from_db(self):
        self.log_info('Initializing DB')
        # Save important properties for reuse
        if self.properties.db_name:
            dbname = self.properties.db_name
        else:
            raise ValueError(
                "Please provide db name using device.load_db('name')")
        ...
        log_warning('Device restored from db')
